...
import { 
  Fingerprint, Camera, ChevronRight, CheckCircle2, User, Clock, 
  Sparkles, Building2, Zap, Shield, LayoutDashboard, Lock, Users, 
  BarChart3, Quote, Menu, X
} from 'lucide-react';
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-b border-slate-100 p-4 space-y-4">
           ...
           <div className="pt-4 border-t border-slate-100 flex flex-col space-y-3">
             <button onClick={() => { navigate('/admin/login'); setMobileMenuOpen(false); }} className="text-left font-semibold text-slate-800">Sign in (Admin)</button>
             <button onClick={() => { navigate('/student/signup'); setMobileMenuOpen(false); }} className="text-left font-semibold text-indigo-600">Get Started</button>
           </div>
        </div>
      )}
    </nav>
